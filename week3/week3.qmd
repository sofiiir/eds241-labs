...
weekly_la_phx <- did_data_az_ca |> 
  mutate(
    date_obj = mdy(date),
    week = floor_date(date_obj, unit = "week") # assign monday to the data
  ) |> 
  filter(year == 2020, month(date_obj) %in% c(2,3,4)) |>  # select for febraury, march, and april
  group_by(week, city) |> 
...
**The parallel trends assumption is violated since before the lockdown the trends are different between Phoenix and LA. These cities NO2 levels cannot be compared.**
...
**There are similar trends in travel and energy usage between these two cities because they have similar conditions. The parallel trends assumption is followed as shown in the pre-lockdown graph trends. **
...
Lockdown in the control, New Mexico causes a 3.5162 reduction in ppb NO2 relative to the control group in Texas between February and April.
...
**Yes, the fluctuations are there but they fluctuate together. The parallel trends assumption is met so Texas makes a good counterfactual for DiD. The volatility of the data influences the effectiveness of the model predictions. This creates an apples to apples comparison.**
...
**The interactive term is the treatment effect. It is not statistically significant. We fail to reject the null hypothesis that lockdown has no effect on the concentration of NO2 ppb for the months February, March, and April.**
...
**A neighboring city in Texas serves as the counterfactual. If the effect of the dust storm is the same for both cities that DiD estimate is not ruined as the same conditions are expereinced in the control and treatment groups.**
...
**The DiD estimate would get smaller because both cities would experience a reduction in traffic which would lead to the same reduction in NO2.**