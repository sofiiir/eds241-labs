...
Treat (-24.77): The treatment reduced the average number of trash piles by approximately 25 piles compared to the control neighborhoods. 
...
post_treat_subset |> 
  ...
  geom_histogram(binwidth = 5, fill = "hotpink", color = "white", alpha = 0.7) +
...
```
Red and green lines do not match. So the variance and the mean are not equal. This shows that the residuals are over dispersed. 
```{r}
...
```
```{r}
trash_piles <- exp(4.36)
pile_reduct <- (exp(-0.38) - 1) *100
se_coef <- exp(0.04)
```
**Intercept:** On average without treatment the model estimates out control group to have 78 waste piles.

**Coefficient:** The model estimates that the treatment reduced waste burning piles by 32% in treated neighborhoods. This is plus or minus approximately 1%. 
...
```
The overdispersion is above 3 therefore this is a large amount of overdispersion. 
...
```
```{r}
trash_piles_reg <- exp(4.31)
pile_red_reg <- (exp(-0.42) -1) * 100
se_coef_reg <- exp(0.12)
```
**Intercept:** On average, there are approximately 74 waste piles in the control group/ untreated neighborhoods. 

**Coefficient:** On average, there was a 35% reduction in waste piles in the treated neighborhoods. 
...
```
The treatment group already had a lower average of waste pile counts before the experiment. To account for this difference it will be added to the model. 
...
```{r}
trash_piles_pre <- exp(0.47)
pile_red_pre <- (exp(-0.27) - 1) * 100

```
**Intercept:** Our model when the pre-count average is 1 and the control group

**Treat:** On average, 

**Pre-count average:** When there is a 1% increase in the pre-count-average 
