...
**RESPONSE:** We minimize variance in unseen covariance. Some covariance that can be fundamental differences are political ideologies and income. 
...
**RESPONSE:** We aim to find a synthetic control that closely tracks the treated unit in the pre-treatment period. If they don't match then we can't utilize it in an as-if method. It would not act as good counterfactual. 
...
**RESPONSE:** We can have similar other covariates but differences in the `co2_power_pc_tons`. We need the covariates to match to create a ceteris paribis and make an apples-to-apples comparison. 
...
**RESPONSE:** There are 49 states in the donor pool. The data covers 23 years.
...
-   **`generate_weights()`**: Chooses donor weights that minimize the distance between California and the synthetic control on the predictors. (optimization)
...
scm_data <- data |> 
  ...
  # defining characteristics synthetic CA must match in order to be considered a good copy of real CA
  generate_predictor(  
    time_window = 2000:2012, # pretreatment period only to compute predictors
    ...
    co2_pre_mean = mean(co2_power_pc_tons, na.rm = TRUE)) |> # average pretreatment outcome
  
  # take the predictor summaries from above and actually figure out how much weight to give each donor state
  # weights each state by covariates 
  generate_weights(
    optimization_window = 2000:2012, # period over which donor weights are optimized
    margin_ipop = 0.02, # smaller equals more precise
    sigf_ipop = 7, # how many significant figs to use 
    bound_ipop = 6) |>  
  
  # construct sythetic CA using optimized donor weight 
  generate_control() 
```
...
**RESPONSE:** The trends track fairly well. The synthetic control is a credible counterfactual. The donors are weighted accurately to make a synthetic control. 

2.  What happens to the gap between the two lines after 2013? What does this suggest about the effect of cap-and-trade?

**RESPONSE:** The gap increases between the two lines after 2013. The effect of cap-and-trade reduces CO2 emissions per capita. 
...
**RESPONSE:** Pre-treatment we expect the value to hover around zero. The standard pre-treatment is between 0.5 and -0.5. The gap increases post-treatment. 
...
scm_data |> 
  plot_placebos() +
  labs(title = "Placebos test",
       y = "CO2 per capita")
```

Does California's gap stand out from the placebo distribution after 2013?

**RESPONSE:** CA post treatment stands out from the rest of the tests. All of the data in this graph is synthetic and represents the gaps of all of the states. 
...
**RESPONSE:** The predictors match fairly well. The `elec_price` matches slightly less than the other variables.

2.  How does the synthetic control compare to the simple donor pool average?

**RESPONSE:** The synthetic CA matches more closely than donor CA making it clear that the weights help adjust the averages. 
...
**RESPONSE:** The per capita CO2 is reduced by CA cap-and-trade
...
**RESPONSE:** We have strong pre-treatment fit. We "excluded other states that were treated". There may be spillover. There are favorable placebo comparisons. 
...
**RESPONSE:** The counterfactual would not be accurate if other states also had cap-and-trade programs. 
...
**RESPONSE:** The pre-treatment average was used as a predictor to get better counterfactuals than matching.  
