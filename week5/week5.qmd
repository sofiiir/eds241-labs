...
**RESPONSE**: The red line shows areas outside of the protected areas. The blue lines show the areas inside of the protected areas. The negative correlates with the areas outside of the protected areas and the positive correlates with the areas inside of the protected areas. If there we no discontinuity in the lines it would indicate that protected areas are not effective at preventing deforestation. 
...
**RESPONSE**: Protected areas have on average 0.10 deforestation compared to un-protected areas. The interaction coefficient represents the slope of the protected areas is 0.002 higher than the slope of unprotected areas. 
...
**RESPONSE**: The second model accounts for the weight of the points and gives the points near the cutoff more weight because we are interested in the differences at the cutoff. In this model there are cutoffs where `lm()` does not have cutoffs. The cutoff here is 2.207 as shown in the `BW est. (h)` which uses both 2.207 and -2.207. The `local polynomial regression` does not need to be linear while the OLS model needs to be linear.  
...
**RESPONSE**: The local polynomial plot fits the data better. It allows the lines to be curved and results in different slopes.
...
**RESPONSE**: There is more discontinuity in the simulated data than in the Neal 2024 paper. Specifically, there is more discontinuity in Indonesia and a larger discontinuity in Brazil in the simulated data.  
